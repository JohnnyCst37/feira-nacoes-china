...
- **Tarefa**: Adicionar link MP3 personalizado, volume global, capturar dados de e-mail e permitir exclusão de alunos.
  ...
  - *Ações*:
    ...
    - **Exclusão de Alunos**: Adicionado botão de lixeira (ícone `Trash2` do Lucide) na tabela de acompanhamento do professor em `Admin.tsx` e implementada a função `deleteStudentProgress` no banco de dados.
...
- **Tarefa**: Resolver erro de compilação da build remota, atualizar links de vídeo (Erhu, Tambor e Dragão), corrigir travamento no mini-game do pastel, resolver erros 404 e adicionar escapes de vídeo externos.
  ...
  - *Ações*:
    ...
    - **Ajuste na Exibição dos Vídeos de Recompensa**: Refatorada a tela final de conclusão da jornada (`Trilha.tsx`) para renderizar a lista de 4 vídeos de recompensa imediatamente ao finalizar a trilha, permitindo que os estudantes assistam aos trabalhos dos colegas antes e durante o processo de envio da avaliação por estrelas.
    - **Suporte a Acesso Rápido / Login Anônimo**: Implementado o suporte a login anônimo via Firebase `signInAnonymously` para contornar bloqueios em contas Google institucionais de estudantes. Adicionado formulário de identificação (Nome e E-mail/Turma opcional) na `Home.tsx` que atua tanto em produção (com fallback de ID local se o provedor anônimo estiver desligado) quanto em desenvolvimento.
    - **Ajuste de Regras do Firestore**: Atualizado o arquivo `firestore.rules` para permitir que usuários não autenticados enviem progressos se o UID do documento começar com `anon-` ou `mock-`, garantindo resiliência total contra falhas ou provedores desligados.
    - **Tratamento de Erros e Degradação Suave no Firestore**: Envolvidas todas as chamadas de banco no `db.ts` (`saveStudentProgress`, `getStudentsProgress`, `deleteStudentProgress`) com blocos `try/catch` robustos. Em caso de restrição de administrador do Firebase (`auth/admin-restricted-operation`) ou regras de escrita não publicadas no banco real, o aplicativo desvia o salvamento automaticamente para o `localStorage` do dispositivo e mescla o progresso local com o leaderboard, impedindo travamentos e permitindo que os alunos joguem o quiz e gerem a Carta Mítica perfeitamente.
